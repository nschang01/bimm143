...
```
    ...
    Info: Working in directory /Users/nicolechang/Desktop/github/bimm 143/class13
...
Lets pass these IDs in `keggresids` to the **pathview()** function to
...
We can also do a similar procedure with gene ontology. Let's focus on BP
...
The Cell Cycle, Mitotic pathway has the most significant “Entities
p-value”. Some of the significant pathways are the same and some are
different from the previous KEGG results. The factors that could cause
differences between the two methods is that the Reactome Analysis looks
at under expression while KEGG looks at under expression and over
expression.
